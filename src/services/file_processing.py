...
def process_photo_collection(
    source_dir: Path, output_dir: Path, collection_name: str
) -> dict:
    ...
    for photo_path in photo_files:
        try:
            ...
            link_photo_with_filename(photo_data, full_output)
...
def is_processing_needed(
    full_path: Path, web_path: Path, output_dir: Path, generated_filename: str
) -> bool:
    ...
    thumb_output = (
        output_dir / "thumb" / generated_filename.replace(full_path.suffix, ".webp")
    )
...
def generate_gallery_metadata(
    photos: List[ProcessedPhoto], collection_name: str
) -> GalleryMetadata:
    ...
    settings_data = GallerySettings(
        ...
        target_timezone_offset_hours=getattr(
            settings, "TARGET_TIMEZONE_OFFSET_HOURS", 13
        ),
    ...
    for photo in photos:
        ...
        photo_id = (
            photo.generated_filename.replace(".jpg", "").replace(".jpeg", "")
            if photo.generated_filename
            else ""
        )
        ...
        exif_data = MetadataExifData(
            ...
            camera={"make": photo.camera.make, "model": photo.camera.model},
        ...
        files_data = MetadataFileData(
            ...
            thumb=f"thumb/{photo.generated_filename.replace('.jpg', '.webp').replace('.jpeg', '.webp')}"
            if photo.generated_filename
            else "",
        )
...
def generate_batch_metadata(
    photos: List[ProcessedPhoto], collection_name: str, batch_number: int
) -> dict:
...
def merge_partial_metadata_files(
    output_dir: Path, collection_name: str
) -> GalleryMetadata:
...
def process_dual_photo_collection(
    ...
    matched, full_only, web_only = validate_matching_collections(
        full_source_dir, web_source_dir
    )
    results["validation"] = {
    ...
    if full_only:
        results["errors"].append(
            f"Photos only in full collection: {', '.join(full_only[:5])}"
            + ("..." if len(full_only) > 5 else "")
        )
    if web_only:
        results["errors"].append(
            f"Photos only in web collection: {', '.join(web_only[:5])}"
    ...
    for batch_start in range(0, len(photo_pairs), batch_size):
        ...
        for full_path, web_path in batch_pairs:
            ...
            try:
                ...
                photo_data.deployment_file_hash = (
                    photo_data.file_hash
                )  # Default fallback
                if photo_data.file_hash and photo_data.exif.timestamp:
                    try:
                        ...
                        photo_data.deployment_file_hash = hashlib.sha256(
                            modified_image_bytes
                        ).hexdigest()

                    except Exception as e:
                        print(
                            f"Warning: Could not calculate deployment hash for {full_path.name}: {e}"
                        )
                ...
                if not is_processing_needed(
                    full_path, web_path, output_dir, photo_data.generated_filename
                ):
                ...
                link_photo_with_filename(web_photo, web_output)
        ...
        if current_batch_photos:
            partial_metadata = generate_batch_metadata(
                current_batch_photos, collection_name, batch_number
            )
