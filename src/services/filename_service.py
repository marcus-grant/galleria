    ...
    if existing_filenames:
        ...
        while True:
            ...
            if sequence > 32:  # Safety check for base32 range
                raise ValueError(
                    f"Too many photos with same timestamp and camera: {base_name}"
                )
    else:
        counter_char = LEXICAL_BASE32[0]  # First photo gets '0'

    filename = (
        f"{collection_name}-{timestamp_str}-{camera_code}-{counter_char}{extension}"
    )

    return filename


def get_timezone_from_gps(
...
def generate_batch_filenames(
    photos: list[ProcessedPhoto], collection: str = "gallery"
) -> list[str]:
