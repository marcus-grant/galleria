...
def wedding_photo_collection(tmp_path):
    ...
    for i, (minutes_offset, phase) in enumerate(timeline):
        ...
        exif_dict = {
            ...
            "Exif": {
                piexif.ExifIFD.DateTimeOriginal: photo_time.strftime(
                    "%Y:%m:%d %H:%M:%S"
                ).encode()
            },
            "1st": {},
...
@pytest.mark.skip(reason="Pre-extraction scope. Rewritten near MVP.")
class TestE2EPipeline:
    """End-to-end integration tests for the complete gallery pipeline."""

    def test_process_photos_command_basic_functionality(
        self, wedding_photo_collection, tmp_path
    ):
    ...
    def test_process_photos_with_timezone_correction(
        self, wedding_photo_collection, tmp_path, monkeypatch
    ):
    ...
    def test_process_photos_performance_with_progress_reporting(
        self, wedding_photo_collection, tmp_path
    ):
        ...
        assert processing_time < 10, (
            f"Processing took {processing_time:.1f}s, should be under 10s"
        )
    ...
    def test_process_photos_crash_recovery_with_resume_flag(
        self, wedding_photo_collection, tmp_path
    ):
        ...
        assert result.exit_code != 0, (
            "Should fail when partial files exist without --resume flag"
        )
        assert (
            "partial" in result.output.lower() or "resume" in result.output.lower()
        ), "Should mention partial files and suggest --resume flag"

    def test_process_photos_restart_flag_cleans_partials(
        self, wedding_photo_collection, tmp_path
    ):
        ...
        if result.exit_code == 0:
            ...
            if partial1.exists():
                ...
                assert '{"partial": "data1"}' not in content, (
                    "Old fake partial file content should be replaced"
                )
            if partial2.exists():
                content = partial2.read_text()
                assert '{"partial": "data2"}' not in content, (
                    "Old fake partial file content should be replaced"
                )
    ...
    def test_process_photos_batch_size_option(self, wedding_photo_collection, tmp_path):
        ...
        if result.exit_code == 0:
            ...
            assert len(partial_files) >= 3, (
                "Batch processing should create multiple partial files"
            )
