...
def create_fake_photo_with_exif():
    ...
    def _create(
        photo_path, timestamp_str=None, camera_make=None, camera_model=None, subsec=None
    ):
        img = Image.new("RGB", (100, 100), color="red")

        if HAS_PIEXIF and (
            timestamp_str or camera_make or camera_model or subsec is not None
        ):
            ...
            if subsec is not None:
                exif_dict["Exif"][piexif.ExifIFD.SubSecTimeOriginal] = str(
                    subsec
                ).encode()
